...
# in this script we will read in the raw club data, 
...
club_id <- club_str[5] #### RESTART FROM HERE #####
...
time_ride <- array( c( clubdata[, 'time_ride_1.2019'], clubdata[, 'time_ride_2.2019'], clubdata[, 'time_ride_3.2019'], clubdata[, 'time_ride_4.2019'], clubdata[, 'time_ride_5.2019'], clubdata[, 'time_ride_6.2019'], clubdata[, 'time_ride_7.2019'], clubdata[, 'time_ride_8.2019'], clubdata[, 'time_ride_9.2019'], clubdata[, 'time_ride_10.2019'], clubdata[, 'time_ride_11.2019'], clubdata[, 'time_ride_12.2019']),
                    dim = c( size, 1, n_waves ) )
df <- data.frame(matrix(time_ride))
df$matrix.time_ride. <- ifelse(df$matrix.time_ride. == 0, NA, df$matrix.time_ride.) #turn 0's to NA
df <- mutate(df, cat = ntile(df$matrix.time_ride., 8))
df$cat <- ifelse(is.na(df$cat), 0, df$cat)
hist(df$cat)
time_ride_temp <- array(df$cat, dim = c(size, 1, n_waves))
#table(time_run_temp)
...
freq_ride_temp <- ifelse(frequencies > 7, 8, frequencies)
...
club <- list("friendship" = friend_data, 
             ...
             "freq_run" = freq_run_temp, "time_run" = time_run_temp, "freq_ride" = freq_ride_temp, "time_ride" = time_ride_temp, "freq_other" = freq_other_temp, "time_other" = time_other_temp, "winter" = winter, "male" = male, "female" = female, "other" = other, "netsize" = size, "novice" = novice)
...
{
  club1 <- load("club19037.RData")
  club1 <- club
  
  club2 <- load("club73971.RData")
  club2 <- club
  
  club3 <- load("club147521.RData")
  club3 <- club
  
  club4 <- load("club324488.RData")
  club4 <- club
  
  club5 <- load("club473939.RData")
  club5 <- club
}
# and make a list containing all the clubdata
