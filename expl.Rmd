...
freq_discount <- varCovar(club$freq_other[,,])
vol_discount <- varCovar(club$time_other[,,])
...
mydata <- sienaDataCreate(kudonet, freq_run, time_run, freq_other, time_other, gender, winter, friendship, freq_discount, vol_discount)
```
...
**Update**: currently attempting to program a variant of the average alter effect.
...
We will check if the model also fits the ego-centered data.

<br>

## 1.5 degree

We start with the 1.5 degree network. Later on we can do a multigroup analysis and extend to the [2.5 degree](#twfive) networks. Egodata of ego 3, 8, 10, and 11 seem sufficient for estimating the SAOM in R-SIENA (i.e. appropriate Jaccard indices, sufficient degrees/density, attribute change etc.). 
...
ego <- egodata1.5[[1]] # grab ego 

# specify variable roles for RSiena object

kudonet <- sienaDependent(ego$kudo[,, c(1:11)])
#kudonet2 <- sienaDependent(ego$kudo2[,, 1:11]) 

freq_run <- sienaDependent(ego$freq_run[,, c(1:11)], type= "behavior")
time_run <- sienaDependent(ego$time_run[,, c(1:11)], type = "behavior")

freq_other <- varCovar(ego$freq_other[,,c(1:11)])
time_other <- varCovar(ego$time_other[,,c(1:11)])
...
winter <- varCovar(ego$winter[,c(1:11)])
...
##### Ego 3
```{r, echo=F}
htmltools::includeHTML("files/ego results/ego3.html")
```

##### Ego 8
```{r, echo=F}
htmltools::includeHTML("files/ego results/ego8.html")
```

##### Ego 10
```{r, echo=F}
htmltools::includeHTML("files/ego results/ego10.html")
```

##### Ego 11
```{r, echo=F}
htmltools::includeHTML("files/ego results/ego11.html")
```
...
Now let's estimate the same model with the 2.5 degree ego-centered networks of the same egos.
Starting with ego 3, and only three timepoints, given the large size of the 2.5 degree network (N=852; ego 3).
...
```{r class.source = 'fold-hide'}
load("egodata2.5.Rdata") # load (raw) 2.5 degree ego-centered data list
ego <- egodata2.5[[8]] # grab ego 

# specify variable roles for RSiena object
kudonet <- sienaDependent(ego$kudo[,, c(1:3)])
#kudonet2 <- sienaDependent(ego$kudo2[,, 1:11]) 

freq_run <- sienaDependent(ego$freq_run[,, c(1:3)], type= "behavior")
time_run <- sienaDependent(ego$time_run[,, c(1:3)], type = "behavior")

freq_other <- varCovar(ego$freq_other[,,c(1:3)])
time_other <- varCovar(ego$time_other[,,c(1:3)])
...
winter <- varCovar(ego$winter[,c(1:3)])
...
print01Report(mydata, modelname="files/test2.5")
```

![](files/test2.5.txt){#id .class width=100% height=200px}


### Step 3: Define effects

```{r echo=T, class.source = 'fold-hide', results='hide'}
myeff <- getEffects(mydata)
myeff1 <- includeEffects(myeff, gwespFF, name = "kudonet") 
myeff1 <- includeEffects(myeff1, outActSqrt, inPopSqrt, name = "kudonet")
myeff2 <- includeEffects(myeff1, egoX, altX, simX, name="kudonet", interaction1 = "freq_run")
