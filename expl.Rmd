...
- For now we take as our (explanatory) network variable the Kudo-network in which awarding/receiving *at least* 1 Kudo constitutes an *i,j* tie. Later on, we use the [ordered variable](#lvl2): in which the second-order network in which awarding/receiving more than 1 Kudo constitutes the presence of a tie, is a sub-network of the first-order network.
...
We may also want to include friendship, as a constant dyadic covariate?

---> 

```{r}
load("clubdata.Rdata") # load (raw) club list
club <- clubdata[[3]] # grab club 
...
First, we are going to include structural network effects, guided by recommendations of @snijderspres: outdegree, reciprocity, and transitivity (GWESP), indegree popularity (sqrt.), outdegree activity (sqrt.), and we may want to add the transitive reciprocated triplets effect; see @block2015.
...
myeff2 <- includeEffects(myeff2, simX, name="kudonet", interaction1 = "freq_run")
myeff2 <- includeEffects(myeff2, simX, name="kudonet", interaction1 = "time_run")
...
#JT I like the code below. Please save the 'final' ans objects somewhere. 
...
#RF in the files folder sub-folders are made for each club, with preliminary results (final.html) in the 'prelim' folder
---> 
...
We present the results for club 1 (N=30), club 2 (N=62), and club 5 (N=77). We also show results for club 3 (N=165) and club 4 (N=13).
...
### Club 1
```{r, echo=F}
htmltools::includeHTML("files/club 1/prelim/final.html")
```

### Club 2
```{r, echo=F}
htmltools::includeHTML("files/club 2/prelim/final.html")
```

### Club 4
```{r, echo=F}
htmltools::includeHTML("files/club 3/prelim/final.html")
```

### Club 4
```{r, echo=F}
htmltools::includeHTML("files/club 4/prelim/final.html")
```
...
### Club 5
```{r, echo=F}
htmltools::includeHTML("files/club 5/prelim/final.html")
```

## {-}

We see a marginal reciprocated degree effect on running volume (club 1) and running frequency (club 2), but not in club 5. In club 5 we find a significant and positive parameter estimate for the average reciprocated alter effect on behavior. This model specification seems to work.



We find average reciprocated alter effect in club 3, for both behaviors. No influence effects were found for club 4.

These results demonstrate that in the clubs under investigation (excluding club 4) influence effects are at play. In two clubs, reciprocating Kudos spurs activity, and in two clubs actors assimilate to the average behavior of their reciprocated ties (indicating support and comparison respectively). These effects are observed even after controlling for selection effects and structural network effects.
