...
siena.table(ansM2, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "last.html", sep = ""))
...
while (TRUE){
  ...
  if(ansM2$tconv.max >= .25){
    ...
    siena.table(ansM2, type="html", tstat=T, d=3, sig=T, paste("files", "/", "test club 1", "/", "ansM2_", try, ".html", sep=""))
    
  }else{
    siena.table(ansM2, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "finalM2", ".html", sep=""))
...
This took some time. So, we also save the sienaFit-object to our "Test Club 1"-folder.
...
Now follow the same procedure for the model comprising also the assimilation- (and outdegree-) effect. Again, we will run the model as many times as necessary, until we reach convergence. 

```{r eval= F }
try <- 1

ansM3 <- siena07(myalgorithm, data = mydata, effects = myeff4, returnDeps=TRUE)
siena.table(ansM3, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "last.html", sep = ""))
...
while (TRUE){
  ...
  if(ansM3$tconv.max >= .25){
    ...
    siena.table(ansM3, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "ansM3_", try, ".html", sep=""))
    
  }else{
    siena.table(ansM3, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "finalM3", ".html", sep=""))
...
The model reached a convergence of .26. We find this acceptable. We save the sienaFit-object.
...
```{r, echo=F}
htmltools::includeHTML("files/test club 1/final.html")
```

### The upward assimilation effect
```{r, echo=F}
htmltools::includeHTML("files/test club 1/final2.html")
```
