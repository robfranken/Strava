...
We read in the R-SIENA objects list (*clubdata_rsiena.RData*) and we grab club 1 (N=30). For now we take as our network variable the kudos-network in which awarding/receiving *at least* 1 kudos constitutes an *i,j* tie. 

- Our (dependent) behavioral variable is running frequency (in times per week; ranging from 0 to 7+ times per week); running duration will be investigated later on.
...
- the interdependence between running and other sports (frequency)
- gender on behavior
...
2. The average attraction towards higher (and lower) effect.
This test the extent to which ego is pulled up (or drawn down) on the behavior scale by alters that score higher (or lower).
...
Second, we add the average attraction higher (lower) effect.
...
while (TRUE){
  ...
  if(ansM2$tconv.max >= .25){
    ...
    siena.table(ansM2, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "Model 1", ".html", sep=""))
...
We also save the sienaFit-object to our "Principal club"-folder.

```{r eval= F }
save(ansM2, file= paste("files", "/", "test club 1", "/", "model1.RData", sep=""))
```


<br>

Now follow the same procedure for the model comprising also the assimilation-effects. Again, we will run the model as many times as necessary, until we reach convergence. Again, we use prevAns.
...
while (TRUE){
  ...
  if(ansM3$tconv.max >= .25){
    ...
    siena.table(ansM3, type="html", tstat=T, d=3, sig=T, file = paste("files", "/", "test club 1", "/", "Model 2", ".html", sep=""))
...
```{r eval= F }
save(ansM3, file= paste("files", "/", "test club 1", "/", "model2.RData", sep=""))

```

<br> 

Because we are now modeling the evolution of both the network and the attribute (running activity), we will add an additional indicator to evaluate GOF; namely, does the model capture the distribution of actors’ attribute levels over time?
...
The first table (Model 1) presents the parameter estimates for (1) network structure; (2) network dynamics (selection); (3) behavior dynamics comprising shape effects, *indegree effect* on behavior, and covariate effects on behavior change. The second table (Model 2) adds the assimilation effects on behavior.

## {.tabset .tabset-fade}

### Model 1
```{r, echo=F}
htmltools::includeHTML("files/test club 1/Model 1.html")
```
...
We do not find a significant kudos indegree effect. We do find that actors are influenced by the behavior of kudos (out-) ties, but they are mostly pulled down (avAttLower)
