...
In the following script we will do the same for our ego-centered data (egodata.RData). Note that we set non-existing (Kudo) ties between actors *i* and *j* with a distance >2 in their friendship ties to *structural zeros* (**NOT YET**). 
...
# the following script reads the data of the egos from the folder for each ego, stores them in a list, and saves it in an object in the last function call of this script. 
  for (i in (1: length(user_str))) {
    ...
    # in case we want to condition on the first activity recorded by users:
    st_yr <- user_data[,'start_date_year']
    novice <-  ifelse(user_data[,'start_date_year'] <= 2011, 9, st_yr)
    novice <-  ifelse(user_data[,'start_date_year'] == 2012, 8, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2013, 7, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2014, 6, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2015, 5, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2016, 4, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2017, 3, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2018, 2, novice)
    novice <-  ifelse(user_data[,'start_date_year'] == 2019, 1, novice)
    ...
    # create list containing all egonet data
    ego <- list("friendship" = friend_data, 
                 "kudo" = kudo_data, "kudo2" = kudo_data2, "kudo3" = kudo_data3,
                 "freq_run" = freq_run_temp, "time_run" = time_run_temp, "freq_other" = freq_other_temp, "time_other" = time_other_temp, "winter" = winter, "male" = male, "female" = female, "other" = other, "netsize" = size, "exp" = novice)
    
    # save 
    save(ego, file=paste("egonets/", "ego", i, ".RData", sep = ""))
...
{
  load("egonets/ego1.RData")
  ego1 <- ego
  
  load("egonets/ego2.RData")
  ego2 <- ego
  
  load("egonets/ego3.RData")
  ego3 <- ego
  
  load("egonets/ego4.RData")
  ego4 <- ego
  
  load("egonets/ego5.RData")
  ego5 <- ego
  
  load("egonets/ego6.RData")
  ego6 <- ego
  
  load("egonets/ego7.RData")
  ego7 <- ego
  
  load("egonets/ego8.RData")
  ego8 <- ego
  
  load("egonets/ego9.RData")
  ego9 <- ego
  
  load("egonets/ego10.RData")
  ego10 <- ego
  
  load("egonets/ego11.RData")
  ego11 <- ego

}
# and make a list containing all the egodata
egodata <- list(ego1, ego2, ego3, ego4, ego5, ego6, ego7, ego8, ego9, ego10, ego11)

# save the output
save(egodata, file="egonets/egodata.RData")
