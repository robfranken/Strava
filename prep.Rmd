...
for (i in (1: length(club_str))) {
  ...
  hours <- ceiling(time_run_h/4) # hours per week
  time_run_temp <- ifelse(hours > 3, 3, hours ) # cap off at 3 hours per week
  ...
  hours <- ceiling(time_other_h/4) # hours per week
  time_other_temp <- ifelse(hours > 3, 3, hours ) # cap off at 3 hours per week
