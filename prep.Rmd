...
{
  load("clubs/clubid1.RData")
  club1 <- club
  
  load("clubs/clubid2.RData")
  club2 <- club
  
  load("clubs/clubid3.RData")
  club3 <- club
  
  load("clubs/clubid4.RData")
  club4 <- club
  
  load("clubs/clubid5.RData")
  club5 <- club
}
...

{
  ...
  clubdata_rsiena2 <- list() # here we take as a cut-off value for kudos-ties 2 kudos from   i to j
  clubdata_rsiena3 <- list() # and here 3
}
...
for (i in 1:5) { 
  ...
  kudonet2 <- sienaDependent(club$kudo2, allowOnly = FALSE) #>1 Kudos
  kudonet3 <- sienaDependent(club$kudo3, allowOnly = FALSE) #>2 Kudos
  
  # NOTE: in some groups, dependent variables (either the kudos network or running activity variables) only have upward or downward changes. We lift the automatic restriction to  follow this pattern in the simulations, by using 'allowOnly = FALSE'. This must be done for the meta-analysis.
  ...
  mydata2 <- sienaDataCreate(kudonet2, friendship, time_run, freq_run, time_other, freq_other, gender, winter)
...
# save the output
{
  ...
  save(clubdata_rsiena2, file="clubs/clubdata_rsiena2.RData")
