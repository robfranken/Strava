...
<!--- RF: hier gaat nog iets niet helemaal goed: hij geeft prints van club 1 voor alle clubs. --> 
...
For the egonets, we went two levels deep: 1) we mapped all the others Strava users that are followed by ego, 2) and all the Strava users that are followed by ego's alters (i.e. "friends of friends"); we mapped all ties defined on level 2. 

we mapped all ties defined on this set (which we define as a 2.5 degree ego-centered network).
...
colnames(e) <- c("i", "j")
e$alter <- ifelse(e$i == "ego", 1, 0) #if ego i follows actor j, j is ego's alter
vertex_attr(G)$name <- ifelse(vertex_attr(G)$name %in% e$j[e$alter==1], "alter", vertex_attr(G)$name)

# give actor attributes
vertex_attr(G)$shape <- c("square", rep("circle", (nrow(fnet) - 1))) #ego is squared
vertex_attr(G)$size <- c(8, rep(4, (nrow(fnet) - 1))) # and bigger
vertex_attr(G)$color <- ifelse(vertex_attr(G)$name == "ego", "#BEBADA", ifelse(vertex_attr(G)$name == "alter", "#FFFFB3", "#8DD3C7"))
