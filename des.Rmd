...
We will use the RSiena object to describe the friendship and kudo networks, constant covariates and behavioral variables. 
...
* behavior: activity level (frequency and volume) and correlation
...
plot_mean_freq <- ggplot(plot.data, aes(month, mean_freq, group = class, colour = class)) +
  ...
  plot_mean_time <- ggplot(plot.data, aes(month, mean_time, group = class, colour = class)) +
    ...
    theme_bw(base_family="serif", base_size=16) +
    scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9")) +
    labs(y = "Weekly running duration in half hours", x="Month") +
    scale_x_discrete(limits=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")) +
    ylim(0,7) +
    theme(axis.text.x = element_text(family="serif", size = 10, angle = 45, hjust = 1), 
          legend.position = "right", legend.title=element_blank()
...
hist(df$depvars$time_run, main = "Histogram of Running Volume", xlab = "Half hours per week", ylab = "count", col = "grey", border= "black", breaks = (max(df2$time_run)-min(df2$time_run)+1))
...
Let's plot the relation between frequency in times per week and volume in half hours per week. We also calculate Kendall's tau-b, i.e. a non-parametric measure of correlation on ranks (cf. @khamis).
...
plot(df, type="b", 
     ...
     sub = "Note: running frequency capped with a maximum of 7, half hours with a maximum of 7",
     xlab = "half hours per week", ylab = "sessions per week",
     col="blue", lwd=c(1,rep(6,10000))) + 
  text(x = 1.5, y = 5, round(c$estimate, digits=2)) +
  text(x = .5, y = 5, "Kendall's tau-b =")
