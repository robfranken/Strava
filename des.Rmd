...
plot_mean_freq <- ggplot(plot.data, aes(month, mean_freq, group = class, colour = class)) +
  ...
  plot_mean_time <- ggplot(plot.data, aes(month, mean_time, group = class, colour = class)) +
    ...
    theme_bw(base_family="serif", base_size=16) +
    scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9")) +
    labs(y = "Running hours per week", x="Month") +
    scale_x_discrete(limits=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")) +
    ylim(0,4) +
    theme(axis.text.x = element_text(family="serif", size = 10, angle = 45, hjust = 1), 
          legend.position = "right", legend.title=element_blank()
...
Let's plot the relation between frequency in times per week and volume in hours per week. We also calculate Kendall's tau-b, i.e. a non-parametric measure of correlation on ranks @khamis.
...
plot(df, type="b", 
     ...
     sub = "Note: running frequency capped with a maximum of 7, hours with a maximum of 3",
     xlab = "hours per week", ylab = "sessions per week",
     col="blue", lwd=c(1,rep(6,10000))) + 
  text(x = 1, y = 5, round(c$estimate, digits=2)) +
...
#RF: How does this happen? Running 3+ hours in one session? Do we miss something...?
