...
We will use the RSiena objects of clubs to describe the friendship and kudo networks, constant covariates and behavioral variables. 
...
* network structure in Friendship and Kudos networks
...
We describe the friendship network. Note that the friendship network is a constant dyadic covariate.
...
G1 <- igraph::graph_from_adjacency_matrix(clubdata_rsiena_freq[[1]]$dycCovars$friendship, mode = "directed", weighted = NULL, diag = TRUE, add.colnames = NA, add.rownames = NA)
...
```{r }
# make igraph object for the club
G1 <- igraph::graph_from_adjacency_matrix(clubdata_rsiena_freq[[2]]$dycCovars$friendship, mode = "directed", weighted = NULL, diag = TRUE, add.colnames = NA, add.rownames = NA)
...
```{r}
# make igraph object for the club
G1 <- igraph::graph_from_adjacency_matrix(clubdata_rsiena_freq[[3]]$dycCovars$friendship, mode = "directed", weighted = NULL, diag = TRUE, add.colnames = NA, add.rownames = NA)
...
```{r }
# make igraph object for the club
G1 <- igraph::graph_from_adjacency_matrix(clubdata_rsiena_freq[[4]]$dycCovars$friendship, mode = "directed", weighted = NULL, diag = TRUE, add.colnames = NA, add.rownames = NA)
...
```{r}
# make igraph object for the club
G1 <- igraph::graph_from_adjacency_matrix(clubdata_rsiena_freq[[5]]$dycCovars$friendship, mode = "directed", weighted = NULL, diag = TRUE, add.colnames = NA, add.rownames = NA)
...
Let's start with describing the total density and intra- (same gender) and intergroup (different gender) densities. We also calculate the Coleman Homophily index for gender, which reflects gender segregation while taking into account the relative group size of gender categories. We describe these segregation measures for friendship and kudo-networks respectively. 
...
Density in friendships is not much higher within-gender than between-genders, though this ratio differs per club (and in some clubs between-gender friendships are more common). The same pattern holds for Kudos-relationships. 

Coleman's Homophily indices vary across clubs. In some clubs there is slight gender segregation (or same-gender preference) in friendship ties (values higher than 0, but rather close to 0), though in others members tend to befriend others outside their own gender (values lower than 0). Overall, tendencies towards segregation (or avoidance thereof) are pronounced to greater extent in the Kudos-relationships than in friendships. 
...
#{r class.source = 'fold-hide'}
df <- clubdata_rsiena_freq[[1]] # grab club 
df1 <- clubdata_rsiena_vol[[1]] # grab club 
df2 <- clubdata[[1]]
...
# create wide dataframe
  df2long <- data.frame (
    gender = rep(c(1,2),15),
    id = c(1:length(df$nodeSets$Actors)),
    ...
    time1 = df1$depvars$time_run[,,1],
    time2 = df1$depvars$time_run[,,2],
    time3 = df1$depvars$time_run[,,3],
    time4 = df1$depvars$time_run[,,4],
    time5 = df1$depvars$time_run[,,5],
    time6 = df1$depvars$time_run[,,6],
    time7 = df1$depvars$time_run[,,7],
    time8 = df1$depvars$time_run[,,8],
    time9 = df1$depvars$time_run[,,9],
    time10 = df1$depvars$time_run[,,10],
    time11 = df1$depvars$time_run[,,11],
    time12 = df1$depvars$time_run[,,12])
...
# add months
plot.data$month <- rep(c("Dec", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov"), 3)
...
plot_mean_freq <- ggplot(plot.data, aes(month, mean_freq, group = class, colour = class)) +
  ...
  plot_mean_time <- ggplot(plot.data, aes(month, mean_time, group = class, colour = class)) +
    ...
    theme_bw(base_family="serif", base_size=16) +
    scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9")) +
    labs(y = "weekly running duration in hours", x="Month") +
    scale_x_discrete(limits=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")) +
    ylim(0,7) +
    theme(axis.text.x = element_text(family="serif", size = 10, angle = 45, hjust = 1), 
          legend.position = "right", legend.title=element_blank()
...
hist(df1$depvars$time_run, main = "Histogram of Running Volume", xlab = "Hours per week", ylab = "count", col = "grey", border= "black", breaks = (max(df2$time_run)-min(df2$time_run)+1))
...
#{r class.source = 'fold-hide'}
df <- clubdata_rsiena_freq[[2]] # grab club 
df1 <- clubdata_rsiena_vol[[2]] # grab club 
df2 <- clubdata[[2]]
...
#{r class.source = 'fold-hide'}
df <- clubdata_rsiena_freq[[3]] # grab club 
df1 <- clubdata_rsiena_vol[[3]] # grab club 
df2 <- clubdata[[3]]
...
#{r class.source = 'fold-hide'}
df <- clubdata_rsiena_freq[[4]] # grab club 
df1 <- clubdata_rsiena_vol[[4]] # grab club 
df2 <- clubdata[[4]]
...
#{r class.source = 'fold-hide'}
df <- clubdata_rsiena_freq[[5]] # grab club 
df1 <- clubdata_rsiena_vol[[5]] # grab club 
df2 <- clubdata[[5]]
...
# create wide dataframe
  df2long <- data.frame (
    id = 1:length(df$nodeSets$Actors),
    gender = gender,
    freq1 = df2$freq_other[,,1],
    freq2 = df2$freq_other[,,2],
    freq3 = df2$freq_other[,,3],
    freq4 = df2$freq_other[,,4],
    freq5 = df2$freq_other[,,5],
    freq6 = df2$freq_other[,,6],
    freq7 = df2$freq_other[,,7],
    freq8 = df2$freq_other[,,8],
    freq9 = df2$freq_other[,,9],
    freq10 = df2$freq_other[,,10],
    freq11 = df2$freq_other[,,11],
    freq12 = df2$freq_other[,,12],
    time1 = df2$time_other[,,1],
    time2 = df2$time_other[,,2],
    time3 = df2$time_other[,,3],
    time4 = df2$time_other[,,4],
    time5 = df2$time_other[,,5],
    time6 = df2$time_other[,,6],
    time7 = df2$time_other[,,7],
    time8 = df2$time_other[,,8],
    time9 = df2$time_other[,,9],
    time10 = df2$time_other[,,10],
    time11 = df2$time_other[,,11],
    time12 = df2$time_other[,,12])
...
plot_mean_freq <- ggplot(plot.data, aes(month, mean_freq, group = class, colour = class)) +
  ...
  plot_mean_time <- ggplot(plot.data, aes(month, mean_time, group = class, colour = class)) +
    ...
    theme_bw(base_family="serif", base_size=16) +
    scale_colour_manual(values=c("#000000", "#E69F00", "#56B4E9")) +
    labs(y = "Other volume per week", x="Month") +
    scale_x_discrete(limits=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")) +
    ylim(0,7) +
    theme(axis.text.x = element_text(family="serif", size = 10, angle = 45, hjust = 1), 
          legend.position = "right", legend.title=element_blank()
...
# histogram (assuming independent observations)
hist(df$vCovar$freq_other, main = "Histogram of Other Frequency", xlab = "Times per week", ylab = "count", col = "grey", border= "black")

hist(df$vCovar$time_other, main = "Histogram of Other Volume", xlab = "", ylab = "count", col = "grey", border= "black")
...
## {.tabset .tabset-fade}
We have now covered the sport activity levels of our club-athletes, and the extent to which friendship- and kudos-associations are segregated along gender. Last, we will explore if friendship- and kudos-ties are also segregated along activity levels. Or in other words: do people with similar activity levels tend to socialize to a greater extent - by befriending one another and by exchanging kudos - even when taking into account the opportunity structures for 'interacting' with (dis)similar others? 
...
# calculate Moran's I
# for distance-1 and with distance decay, in the friendship and kudo network, for frequency and volume  respectively
...
rownames(f_mat) <- rownames(v_mat) <- c("direct friends", "direct and indirect friends (distance-decay)", "direct kudos ties", "direct and indirect kudos ties (distance-decay)")
...
colnames(f_mat) <- colnames(v_mat) <- c("observed", "expected", "sd", "p-value")
rownames(f_mat) <- rownames(v_mat) <- c("direct friends", "direct and undirect friends (distance-decay)", "direct kudos ties", "direct and indirect kudos ties (distance-decay)")
...
We observe that, indeed, friends that are closer to one another are more alike, and especially kudos-ties! Autocorrelation was stronger without the distance-decay function, which suggests that especially close alters (with path length one) are similar. 
